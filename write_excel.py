...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 5:
        print("Usage: python screenshot.py <sheet_name> <row> <col> [<text> | clipboard]")
        sys.exit(1)

    excel_file = "your_excel_file.xlsx"  # Specify the path to your Excel file
    sheet_name = sys.argv[1]  # Sheet name
    row = sys.argv[2]  # Row number
    col = sys.argv[3]  # Column letter

    if sys.argv[4].lower() == "clipboard":
        # Get text from clipboard
        try:
        ...
        text = " ".join(sys.argv[4:])  # Join the rest of the arguments as text
